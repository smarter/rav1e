...
use plane::*;
use quantize::*;
use rate::QuantizerParameters;
use rate::FRAME_SUBTYPE_I;
use rate::FRAME_SUBTYPE_P;
use rdo::*;
use segmentation::*;
use transform::*;
use util::*;
...
impl FrameInvariants {
  ...
  pub fn get_frame_subtype(&self) -> usize {
    if self.frame_type == FrameType::KEY {
      ...
      let lvl = if !inter_cfg.reorder {
        ...
        pos_to_lvl(inter_cfg.idx_in_group - inter_cfg.pyramid_depth + 1,
  ...
  pub fn set_quantizers(&mut self, qps: &QuantizerParameters) {
    ...
    self.lambda = qps.lambda*((1 << 2*(self.sequence.bit_depth - 8)) as f64);
...
fn encode_partition_bottomup(
  ...
  if !must_split {
    ...
    if bsize.gte(BlockSize::BLOCK_8X8) && is_square {
      ...
      cost = (w.tell_frac() - tell) as f64 * fi.lambda/ ((1 << OD_BITRES) as f64);
    }
  ...
  if can_split {
    for &partition in RAV1E_PARTITION_TYPES {
      ...
      if bsize.gte(BlockSize::BLOCK_8X8) {
        ...
        rd_cost = (w.tell_frac() - tell) as f64 * fi.lambda/ ((1 << OD_BITRES) as f64);
      }
