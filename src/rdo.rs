...
pub fn compute_distortion_bias<T: Pixel>(
  ...
  let mut total_propagate_cost = 0. as f64;
  let mut total_intra_cost = 0. as f64;
  for y in y1..y2 {
    for x in x1..x2 {
      let intra_cost = fi.lookahead_intra_costs[y / 2 * fi.w_in_imp_b + x / 2] as f64;
      let propagate_cost = fi.block_importances[y / 2 * fi.w_in_imp_b + x / 2] as f64;
      total_intra_cost += intra_cost;
      total_propagate_cost += propagate_cost;
    }
  }
